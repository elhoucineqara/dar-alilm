...
export default function DashboardCourseViewPage() {
  ...
  const [mainSidebarOpen, setMainSidebarOpen] = useState(true); // Main navigation sidebar - open by default
    ...
    try {
      ...
      if (!courseRes.ok) {
        ...
        if (courseRes.status === 403) {
          ...
          setError(errorData.error || 'Unable to load course details');
        }
        setLoading(false);
        return;
      }
      ...
      if (progressRes.ok) {
        ...
        if (progressData.progress) {
          ...
          // Ensure we use the real progress value from the database
          const realProgress = progressData.progress.overallProgress || 0;
          setCourseProgress(realProgress);
          console.log('Course progress loaded:', realProgress);
        } else {
          console.warn('No progress data in response:', progressData);
          setCourseProgress(0);
        }
      } else {
        const errorData = await progressRes.json().catch(() => ({}));
        console.error('Failed to fetch progress:', progressRes.status, errorData);
        setCourseProgress(0);
      }
      
      // Also fetch enrollment status to verify course completion
      try {
        const enrollmentRes = await fetchApi(`/api/student/courses`);
        if (enrollmentRes.ok) {
          const enrollmentData = await enrollmentRes.json();
          const enrolledCourse = enrollmentData.courses?.find((item: any) => item.course._id === courseId);
          if (enrolledCourse) {
            // Update progress from enrollment data if available
            if (enrolledCourse.progress && enrolledCourse.progress.overallProgress !== undefined) {
              const enrollmentProgress = enrolledCourse.progress.overallProgress || 0;
              setCourseProgress(enrollmentProgress);
              console.log('Course progress from enrollment:', enrollmentProgress);
            }
            // Check enrollment status
            if (enrolledCourse.enrollment && enrolledCourse.enrollment.status === 'completed') {
              setCourseProgress(100);
              console.log('Course marked as completed in enrollment');
            }
          }
        }
  ...
  const updateProgress = async (moduleId?: string, sectionId?: string, quizId?: string, completedFinalExam?: boolean) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.progress && data.progress.overallProgress !== undefined) {
          // Use the real progress value from the API response
          const realProgress = data.progress.overallProgress || 0;
          setCourseProgress(realProgress);
          setProgress(data.progress);
          console.log('Progress updated:', realProgress);
        } else {
          // If progress not in response, fetch it fresh
          const progressRes = await fetchApi(`/api/student/progress?courseId=${courseId}`);
          if (progressRes.ok) {
            const progressData = await progressRes.json();
            if (progressData.progress) {
              ...
              setCourseProgress(realProgress);
              setProgress(progressData.progress);
              console.log('Progress refreshed:', realProgress);
            }
          }
        }
      } else {
        const errorData = await response.json().catch(() => ({}));
        console.error('Failed to update progress:', response.status, errorData);
      }
    } catch (error) {
  ...
  const downloadCertificatePDF = async () => {
    ...
    doc.setFontSize(18);
    doc.setFont('helvetica', 'bold');
    doc.text(course?.title || 'Course', 148.5, 141, { align: 'center' });
    ...
    // Download the PDF
    doc.save(`Certificate-${course?.title || 'Course'}-${studentName}.pdf`);
  ...
  return (
    <>
      ...
      <StudentLayout
        user={user ? { ...user, role: user.role || 'student' } : undefined}
                          ...
                          <button
                            ...
                            onClick={async (e) => {
                                    ...
                                    try {
                                      ...
                                      if (progressRes.ok) {
                                        ...
                                        if (progressData.progress) {
                                          ...
                                          setCourseProgress(realProgress);
                                          setProgress(progressData.progress);
                                          console.log('Final exam completed, progress refreshed:', realProgress);
                                          // If course is 100% complete, show certificate
                                          if (realProgress >= 100) {
                                      ...
                                      console.error('Error refreshing progress after final exam:', error);
                                    }
                                  }, 500);
                                }
                              }
                              ...
                              if (activeQuiz && 'moduleId' in activeQuiz) {
                                ...
                                // Progress should already be updated above, but check again
