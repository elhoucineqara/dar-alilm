...
export default function StudentSettingsPage() {
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profileImage) {
        ...
        const uploadRes = await fetchApi('/api/student/upload', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetchApi('/api/student/change-password', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleNotificationSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetchApi('/api/student/notifications', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        ...
        <form onSubmit={handleNotificationSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between py-3 border-b border-gray-100">
            <div>
              ...
              <p className="text-sm text-gray-500">Receive notifications from instructors</p>
            </div>
            <input
              type="checkbox"
              checked={notificationSettings.instructorMessages}
              onChange={(e) => setNotificationSettings({ ...notificationSettings, instructorMessages: e.target.checked })}
              className="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500 cursor-pointer"
            />
          </div>
