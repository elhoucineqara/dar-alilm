...
// Helper function to get file URL from fileId or fileUrl (for backward compatibility)
...
export default function EditCoursePage() {
  ...
  const handleUpdateSection = async (e: React.FormEvent, sectionId: string) => {
    ...
    try {
      ...
      let fileId = editingSectionData?.fileId || '';
      ...
      const res = await fetchApi(`/api/instructor/sections/${sectionId}`, {
        ...
        body: JSON.stringify({ 
          ...
          type: newSectionType,
          fileId: newSectionType === 'file' ? fileId : undefined,
          fileUrl: newSectionType === 'file' ? fileUrl : undefined, // Keep for backward compatibility
                              ...
                              <form onSubmit={(e) => editingSection ? handleUpdateSection(e, editingSection) : handleAddSection(e, module._id)} className="space-y-3">
                                ...
                                {newSectionType === 'file' && (
                                  <div>
                                    ...
                                    {editingSection && getFileUrl(editingSectionData) && (
                                      <p className="text-xs text-gray-600 mb-2">
                                        Current file: <a href={getFileUrl(editingSectionData)} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{editingSectionData?.fileName || 'View file'}</a>
                                      </p>
                                    )}
                                    <input
                                      type="file"
